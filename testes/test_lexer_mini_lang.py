"""
Script de testes para o Lexer da linguagem Mini-Lang
Testa os casos seguintes:
1. Comentários (//)
    ...
    try:
        ...
        if verbosity == 1:
            ...
            if isinstance(resultado, list):
                print(f"Resultados ({len(resultado)} items):\n")
                for i, item in enumerate(resultado, 1):
                    if hasattr(item, 'type') and hasattr(item, 'value'):
                        # É um token
                        if item.type == TT_STRING:
                            print(f"{i:3d}. [{item.type:12s}] valor: {repr(item.value)}")
                        elif item.value is not None:
                            print(f"{i:3d}. [{item.type:12s}] valor: {item.value}")
                        else:
                            print(f"{i:3d}. [{item.type:12s}]")
                    else:
                        print(f"{i:3d}. {item}")
